...
export const useCategoriesFilter = () => {
    ...
    const getTheFilteredProducts = useCallback(() =>
        allProducts.filter((item) =>
            chosedCategories.items.indexOf(item.data.category.slug) > -1
        ), [allProducts, chosedCategories.items]
    )
    ...
    const handleCheckboxChange = useCallback((event) => {
        ...
        if (!isItOnTheArray && event.target.checked) addCategory(arrayOfchosedCategories, name)

        if (isItOnTheArray && !event.target.checked) removeCategory(arrayOfchosedCategories, name)

        if (chosedCategories.items.length > 0) setShowResetButton(true)
    ...
    }, [chosedCategories.items, getTheFilteredProducts])
    ...
    }, [chosedCategories, areProductsLoaded, allProducts])
